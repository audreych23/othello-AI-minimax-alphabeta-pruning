...
std::vector<std::array<std::array<int, SIZE>, SIZE>>store_board; 
...
/*Node DLS() { 
 ...
 Node IDDFS(int from, int to, int max_depth) { 
     while (time != 10) { 
         //  call the alpha beta + dls 
         DLS(); 
     } 
}*/ 
...
double mobility_heuristic_evaluation_function(std::array<std::array<int, SIZE>, SIZE> currStateBoard/*, int currPlayer*/) { 
    ...
    double heuristic = 10 * (my_tiles - opp_tiles) + (m_player - m_opp); 
    return heuristic; 
...
Node minimax(std::array<std::array<int, SIZE>, SIZE> currStateBoard, int currPlayer, int depth, double alpha, double beta/*, std::array<int, 3> disc_count_state*/) { 
    ...
    if (disc_count[0] > 14) { 
        ...
        //change heuristic when empty <= 14 or 12  
        if (depth == 14) { 
    ...
    // copyboard for this depth 
    /*if (store_board.size() != depth + 1) { 
        std::array<std::array<int, SIZE>, SIZE> temp = copy_board(currStateBoard); 
        store_board.push_back(temp); 
    }*/ 
    if (currPlayer == player) { 
        ...
        for (unsigned long long int i = 0; i < availablePoint.size(); i++) { 
            ...
            //currStateBoard = store_board[depth]; 
            currStateBoard = temp; 
